...
SELECT app_name, category, rating FROM analytics WHERE id = 1880;
...
SELECT app_name, min_installs, rating FROM analytics WHERE rating > 0 AND min_installs <= 50 ORDER BY rating DESC;
    -- Find the names of all apps that are rated less than 3 with at least 10000 reviews.
SELECT app_name, rating, reviews FROM analytics WHERE rating < 3 and reviews >= 10000;
    -- Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
SELECT app_name, price, reviews FROM analytics WHERE price BETWEEN .10 AND 1.0 ORDER BY reviews DESC LIMIT 10;
    -- Find the most out of date app. Hint: You don’t need to do it this way, but it’s possible to do with a 
